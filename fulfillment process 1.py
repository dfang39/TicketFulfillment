...
requests_book = openpyxl.load_workbook('TicketRequests1.xlsx', read_only=True)
request_sheet = requests_book.get_sheet_by_name('Ticket Request Associated View')


class Request(object):
    def __init__(self, id_number="", agency="", tickets_requested="", request_status="", number_children="", number_adults="", ticket_assignments=[]):
...
for row in range(2, request_sheet.max_row + 1):
    request_objects.append(
        Request(request_sheet['D' + str(row)].value, request_sheet['E' + str(row)].value, request_sheet['J' + str(row)].value, request_sheet['G' + str(row)].value, request_sheet['N' + str(row)].value, request_sheet['O' + str(row)].value))
    ...
    if requested_tickets > 0:
        for donation in donation_objects:
            ...
            if donated_tickets == requested_tickets:
                ...
                break
            elif requested_tickets > donated_tickets > 1 and (requested_tickets - donated_tickets) > 1:
